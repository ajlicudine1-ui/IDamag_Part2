...
/**
 * Recover a deterministic aggregate plan when the LLM asks an unnecessary
 * schema clarification even though the current live schema contains one
 * clearly matching numeric metric.
 *
 * Example shape (no business-specific names are hardcoded):
 *   question: "what is the total <metric phrase> ...?"
 *   planner:  { route: "clarify", ... }
 *
 * The recovery is deliberately conservative:
 *   - only SUM / AVERAGE wording is handled here;
 *   - ranking questions are left to the ranking resolver;
 *   - the best numeric schema field must have a strong lexical match;
 *   - ambiguous near-ties stay as clarifications;
 *   - live value filters are inferred, then the normal semantic filter
 *     guard decides whether they are genuine narrowing or descriptive
 *     report-context wording.
 */
function recoverClarifyAggregatePlan({
  datasets,
  schema,
  plan,
  question,
  ...
  const stopTokens = new Set([
    ...
    "the", "a", "an", "of", "for", "to", "from", "in", "on",
    "at", "by", "with", "and", "or", "covered", "cover",
    "overall", "combined", "altogether", "all",
  ]);
  ...
  for (const datasetSchema of schema || []) {
    ...
    for (const column of datasetSchema?.columns || []) {
      ...
      score += overlapRatio * 1.8;

      if (
        compactCore &&
        questionText.includes(compactCore)
      ) {
        score += 1.5;
      }

      // Aggregate words such as "total" often belong to the real metric
      // label itself. Reward, but never require, that exact schema token.
      if (
        requestedAggregation === "sum" &&
        /\btotal\b/.test(normalizedColumn) &&
        /\btotal\b/.test(questionText)
      ) {
        score += 0.35;
      }

      const nonEmptyNumeric =
        rows.reduce(
          (count, row) =>
            looksNumericValue(
              row?.[column.name]
      ...
      score += Math.min(coverage, 1) * 0.15;
  ...
  // Require a strong lexical match and avoid silently choosing between
  // two nearly identical candidates from different worksheets.
  if (
    !best ||
    best.overlapRatio < 0.6 ||
    best.score < 1.35 ||
    (
      second &&
      second.dataset !== best.dataset &&
      second.column !== best.column &&
      Math.abs(best.score - second.score) < 0.18
    )
  ) {
    return plan;
  }
  ...
  let recovered = {
    ...
    limit: 1,
    showAll: false,
    clarifyAggregateRecovered: true,
    clarifyAggregateOriginalQuestion:
      plan.question || null,
  };

  recovered = sanitizeSemanticPlanFilters({
    datasets,
    plan: recovered,
    question,
    reportContext,
