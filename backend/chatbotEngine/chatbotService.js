...
function detectGroupedComparisonOperation(
  ...
  if (
    /\b(sum|total|combined|overall|altogether)\b/.test(
      text
    )
  ) {
    return "group_sum";
...
function repairGroupedComparisonPlan({
  datasets,
  plan,
  question,
}) {
  ...
  /**
   * Preserve the planner's metric column.
   *
   * For aggregate comparisons the two entity values become
   * one IN-filter on their shared grouping column.
   */
  return {
    ...plan,
...
function normalizePlannerPlan({
  ...
  /**
   * Analytical comparison repair must happen before the generic
   * multi-entity lookup logic below.
   *
   * This prevents:
   * "Compare the average X of A and B"
   *
   * from being downgraded to lookup + filterGroups.
   */
  const groupedComparison =
    repairGroupedComparisonPlan({
      datasets,
      plan:
        normalized,
      question,
