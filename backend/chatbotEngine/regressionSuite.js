...
test('problem3 local entity-list planner resolves a filtered descriptive entity without requiring a numeric metric', () => {
  ...
  assert.deepEqual(plan.selectColumns, ['SP ID', 'SP Name', 'Municipality']);
  assert.equal(plan.preserveEntityRecords, true);
...
test('entity list preserves separate records when names repeat but IDs differ', () => {
  const { executePlan } = require('./calculationEngine');
  const datasets = {
    Sheet1: [
  ...
  const plan = {
    ...
    labelColumn: 'SP Name',
    filters: [{ column: 'Province', operator: 'equals', value: 'Pangasinan' }],
    selectColumns: ['SP ID', 'SP Name', 'Municipality'],
    preserveEntityRecords: true, showAll: true, outputRequested: true, limit: 100,
  };
  const result = executePlan({ plan, datasets, question: 'What subprojects are in Pangasinan?' });
