...
function inferEntityRecordProjection(rows, filteredRows, entityColumn, filters = []) {
  if (!Array.isArray(rows) || !rows.length || !entityColumn) return null;

  // Only auto-expand descriptive entity-name/title lists. This keeps ordinary
  // categorical lists (Province, Status, Type, etc.) unchanged.
  const entityName = normalizeText(entityColumn);
  if (!/\b(?:name|title)\b/.test(entityName)) return null;

  const scopedRows = Array.isArray(filteredRows) ? filteredRows : rows;
  if (!scopedRows.length) return null;

  const entityValues = scopedRows
    .map((row) => String(row?.[entityColumn] ?? "").trim())
    .filter(Boolean);

  if (entityValues.length < 2) return null;

  const uniqueEntityValues = new Set(
    entityValues.map((value) => value.toLowerCase())
  );

  // If every entity name is already unique, normal list behavior is sufficient.
  if (uniqueEntityValues.size === entityValues.length) return null;
  ...
  const idCandidates = columns
    ...
    .map((column) => {
      ...
      const idLike =
        /(^|\s)(?:id|identifier|code)(?:\s|$)/.test(normalized);
      ...
      const coverage = values.length / scopedRows.length;
      ...
      return {
        ...
        score:
          uniqueness * 0.70 +
          coverage * 0.20 +
          (normalized.includes("id") ? 0.10 : 0),
        uniqueness,
        coverage,
      };
    })
    .filter(Boolean)
    .filter((item) => item.uniqueness >= 0.90 && item.coverage >= 0.80)
    .sort((a, b) => b.score - a.score);

  const idColumn = idCandidates[0]?.column;
  ...
  for (const key of locationPriority) {
    locationColumn = columns.find((column) => {
      ...
      return (
        ...
        normalizeText(column) !== normalizeText(idColumn) &&
        !filterColumns.has(normalized) &&
  ...
  const selectColumns = [idColumn, entityColumn];
  if (locationColumn && !selectColumns.includes(locationColumn)) {
    selectColumns.push(locationColumn);
  }
...
function executePlan({
  ...
  if (
    ...
    //
    // This is intentionally enforced here as a shared execution invariant, not
    // only in the local planner. Groq may return a valid list plan containing
    // only `SP Name`; if the live rows show duplicate names but a strong unique
    // ID exists, collapsing by name would lose real records.
    const inferredEntityProjection =
      operation === "list"
        ? inferEntityRecordProjection(
            rows,
            filteredRows,
            selectedColumn,
            filters
    ...
    const shouldPreserveEntityRecords =
      operation === "list" &&
      (
        plan.preserveEntityRecords === true ||
        Boolean(inferredEntityProjection)
      );

    if (shouldPreserveEntityRecords) {
      const requestedRecordColumns =
        plan.preserveEntityRecords === true &&
        Array.isArray(plan.selectColumns) &&
        plan.selectColumns.length > 1
          ? plan.selectColumns
          : inferredEntityProjection?.selectColumns || [];

      const selectedColumns = requestedRecordColumns
        .map((name) => findColumn(rows, name))
        .filter(Boolean);

      if (selectedColumns.length < 2) {
        // Fall through to normal distinct-value list behavior if the projection
        // could not be grounded against the live schema.
      } else {
      ...
      return {
        ...
        entityRecordProjectionInferred:
          plan.preserveEntityRecords !== true &&
          Boolean(inferredEntityProjection),
        answer:
